...
property_data <- property_data %>% filter(property_data$suburb == "Manly")
...
df <- property_data %>%
  mutate(
    ...
    no_of_bed_cat  = cut(no_of_bath, breaks = seq(0, 10, by = 2)),
    no_of_bath_cat = cut(no_of_bath, breaks = seq(0, 10, by = 2)),
...
#unique(train_data$distance_station)
...
#Write file as csv
write.csv(test_data, paste("Regression/test_data_",suburb,".csv"), row.names = FALSE)
closeAllConnections()
